...
def run(context):
    ...
    try:
        ...
        #dummy = rootComp.bRepBodies.item(0).boundingBox.
        if cancelled:
        ...
        for x in range(int(amount)):
            # Selecting the bottom edge
            edges = adsk.core.ObjectCollection.create()
            
            edges.add(ui.selectEntity('Edges to Extend', 'CircularEdges').entity)
            min_point = edges.item(0).boundingBox.minPoint.z
            distance_down = stock_height + min_point + z_offset + .01 #ensure a clean breakthrough
        ...
        for body in bodies:
            ...
            body_ycenter = (body_ymax + body_ymin) / 2

            translation_y = -2 * body_ycenter
            
            vector = adsk.core.Vector3D.create(0, translation_y, 0)
